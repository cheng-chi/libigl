...
#else
  const static size_t sthc = [](){
    size_t hw_nthreads = std::thread::hardware_concurrency();
    char* env_str = getenv("IGL_PARALLEL_FOR_NUM_THREADS");
    if (env_str == nullptr){return hw_nthreads;}
    int user_nthreads = atoi(env_str);
    if (user_nthreads < 1){return hw_nthreads;}
    return user_nthreads<hw_nthreads?user_nthreads:hw_nthreads;
  }();
